<!DOCTYPE html>
<html>
<head>
	<title>Linea 7</title>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<link rel="shortcut icon" href="imgs/logo.svg" />
	<link rel="stylesheet" type="text/css" href="css/j-animation.css">
	<link href="https://fonts.googleapis.com/css?family=Raleway" rel="stylesheet"> 
	<link rel="stylesheet" type="text/css" href="css/lineas.css">
</head>
<body>
	<div id="cover">
		<h1>7)	Políticas de seguridad radiológica en asistencia sanitaria.</h1>
	</div>
	<p class="j-animate j-right">
		La seguridad de paciente y del personal ocupacionalmente expuesto, debe ser una prioridad estratégica en los usos de la radiación ionizante en medicina, por tal motivo es necesario promover la cooperación entre autoridades reguladoras de protección radiológica, autoridades de salud, sociedades profesionales y de paciente, entre las diferentes disciplinas y especialidades involucrada en la práctica radiodiagnóstica. El departamento de ITDi SEROFCA, propone:
	</p>
	<figure>
		<img src="l/7.png">
		<figcaption></figcaption>
	</figure>
	<h2>7.1 Desarrollo de guías prácticas para la aplicación de normas.</h2>
	<h2>7.2 Propuestas de normas nacionales de calidad en el diagnóstico médico.</h2>
	<h2>7.3 Normas de protección radiológica en intervencionismo.</h2>
	<h2>7.4 Actualización de normas nacionales.</h2>

	<?php include 'form.php'; ?>
</body>
</html>